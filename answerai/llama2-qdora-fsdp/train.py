...
def fsdp_main(local_rank:int, world_size:int, args:Dict):
    ...
    elif args["train_type"] in ["qlora", "qdora"]:
        # use normal transformers loading for qlora/qdora
    ...
    elif args["train_type"] in ["custom_qlora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]: # Our custom loading
        cfg = AutoConfig.from_pretrained(args["model_name"])
        cfg.use_cache = False
        cfg._attn_implementation = attn_impl
    ...
    if args["train_type"] in ["lora", "qlora", "qdora"]:
        ...
        peft_config = LoraConfig(
            ...
            target_modules="all-linear" if args["lora_target_modules"] == "all" else args["lora_target_modules"],
            use_dora=args["train_type"] == "qdora",
        )
        ...
        # needs autocast_adapter_dtype=False because non-PEFT uses bfloat16 for adapters
        model = get_peft_model(model, peft_config, autocast_adapter_dtype=False)
        _, param_count = model.get_nb_trainable_parameters()
    ...
    if rank == 0 and args['verbose']:
        ...
        print(f"Total model params:     {param_count}")
        print(f"Trainable model params: {trainable_param_count}")
        dtype_set = {p.dtype for p in model.parameters()}
        print(f"Model dtypes:           {dtype_set}")
    ...
    if args["train_type"] in ["qlora", "qdora"]:
        from peft.utils.other import fsdp_auto_wrap_policy
        my_auto_wrap_policy = fsdp_auto_wrap_policy(model)
    ...
    extra_log_path = f"/tmp/answerai/log_{args['train_type']}_{random.randint(0,1000000)}.txt"
    if rank == 0:
        print("Total Training Steps:", num_training_steps)
        print("Extra log path:", extra_log_path)
    memory_stats = []
    ...
    with profiling_context(args, rank=rank) as prof:
        for epoch in range(args['num_epochs']):
            ...
            for batch_idx, batch in enumerate(dataloader):
                extra_logs = {"step": epoch*len(dataloader)+batch_idx}
                ...
                if accumulate_grads:
                    ...
                    # extra_logs["lr"] = lr_scheduler.get_last_lr()[0] if lr_scheduler is not None else args["lr"]
                    # extra_logs["tokens_per_second"] = num_tokens / (time.perf_counter() - tic)
                    # num_tokens = 0
                    # tic = time.perf_counter()
                    # memo = get_memory_stats(torch.device(rank))
                    # extra_logs["peak_memory_active"] = memo["peak_memory_active"]
                    # extra_logs["peak_memory_alloc"] = memo["peak_memory_alloc"]
                    # extra_logs["peak_memory_reserved"] = memo["peak_memory_reserved"]
                    # extra_logs["rank"] = rank
                    # if rank == 0:
                    #     with open(extra_log_path, 'a') as f:
                    #         f.write(json.dumps(extra_logs) + "\n")
                ...
                if accumulate_grads:
                    ...
                    if rank == 0:
                        extra_logs["loss"] = log_loss.item()
                        extra_logs["lr"] = log_lr
                        extra_logs["tokens_per_second"] = num_tokens / (time.perf_counter() - tic)
                        num_tokens = 0
                        tic = time.perf_counter()
                        memo = get_memory_stats(torch.device(rank))
...
def main(
    world_size: int = -1, # Number of GPUs to use. -1 = all available GPUs.
    train_type: Param("", choices=["full", "lora", "qlora", "qdora", "custom_qlora", "custom_lora", "hqq_lora", "hqq_dora", "bnb_dora", "bnb_llama_pro", "hqq_llama_pro"]) = "qlora", # "full", "lora", "qlora", or "custom_qlora"
    llama_pro_path: str = None, # Path to the quantized llama pro model
    batch_size: int = 1, # Batch size per GPU. Effective BS = batch_size * world_size * gradient_accumulation_steps
    context_length: int = 512, # Max length of input sequence (in tokens)
    gradient_accumulation_steps: int = 1, # How many steps to accumulate gradients over (increases effective batch size)
